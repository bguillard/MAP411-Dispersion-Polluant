# -*- coding: utf-8 -*-
"""
Created on Sun Dec 25 19:59:16 2016
...
# Modification de G et D pour la conditiona de type Neumann homogène
G[N-1][N-1]=1.0/dx
G[N-1][N-2]=-1.0/dx
D[N-1]=[0]*N
...
fig = plt.figure()
fig.suptitle('Q8 : Schema de Crank-Nicholson, avec terme source', fontsize=14, fontweight='bold')
...
def animate(i):
	...
	# Si on est a une date paire, on ajoute le terme source
	if int(nbrIter*dt)%2==0:
		Y=lnlg.solve_triangular(B,np.dot(D,U) + S , check_finite=False, lower=True)
	else:
